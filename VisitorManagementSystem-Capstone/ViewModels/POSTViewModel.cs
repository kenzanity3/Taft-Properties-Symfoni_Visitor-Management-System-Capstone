    ...
    public class POSTViewModel
    {
        ...
        public async Task<List<string>> RegisterUserAsync(VisitorManagementSystemDatabaseContext ctx, int mode)
        {
            ...
            try
            {
                ...
                userbundle.User.UserId = await GenerateNextVisitorIdAsync(ctx, prefix);
        ...
        // Add this helper method to your POSTViewModel class:

        /// <summary>
        /// Generates the next sequential Visitor ID with format VIS-YYYY-XXXXX
        /// </summary>
        /// <summary>
        /// Generates the next sequential Visitor ID with format VIS-YYYY-XXXXX
        /// Thread-safe version
        /// </summary>
        public async Task<string> GenerateNextVisitorIdAsync(VisitorManagementSystemDatabaseContext ctx, string prefix)
        {
            ...
            lock (lockObj)
            {
                // Get all existing IDs that match the pattern
                var existingIds = ctx.Users
                    .Where(u => u.UserId.StartsWith(pattern))
                    .Select(u => u.UserId)
                    .ToList();
                ...
                {
                    string numberPart = id.Substring(pattern.Length);
                ...
                return $"{pattern}{(maxNumber + 1).ToString("D5")}";
            }
        }
