...
namespace VisitorManagementSystem_Capstone.Controllers
{
    public class KioskController : Controller
    {
        ...
        public async Task<IActionResult> KioskCheckInOut()
        {
            try
            {
                // Get today's check-ins (only approved and checked-in visitors)
                var checkIns = await _context.VisitLogs
                    ...
                    .Where(v => v.IssueDate == DateOnly.FromDateTime(DateTime.Today) &&
                               ...
                               v.CheckInOut != null) // Only those who have checked in
                    .Select(v => new CheckInOutViewModel
                    {
                        ...
                        CheckOutTime = v.CheckInOut.CheckOutDateTime.HasValue ?
                            v.CheckInOut.CheckOutDateTime.Value.ToString("hh:mm tt") : "Still Checked In"
                    })
                    .ToListAsync();

                // Get today's check-outs (only approved and checked-out visitors)
                var checkOuts = await _context.VisitLogs
                    ...
                    .Where(v => v.IssueDate == DateOnly.FromDateTime(DateTime.Today) &&
                               ...
                               v.VerificationStatus == true && // Only approved appointments
                               v.CheckInOut != null &&
                               v.CheckInOut.CheckOutDateTime.HasValue) // Only those who have checked out
                    .Select(v => new CheckInOutViewModel
                    {
                        VisitorName = v.Visitor.User.FirstName + " " + v.Visitor.User.LastName,
                        CheckInTime = v.CheckInOut.CheckInDateTime.ToString("hh:mm tt"),
            ...
            {
                // Log error
                Console.WriteLine($"Error loading check-in/out data: {ex.Message}");
        ...
        public async Task<IActionResult> ProcessCheckIn([FromBody] CheckInRequest request)
        {
            try
            {
                ...
                else
                {
                    ...
                    Console.WriteLine($"Check occupant: {checkoccupant?.RoomOwner?.User?.FirstName} {checkoccupant?.RoomOwner?.User?.LastName}");
                    ...
                    // Validate visit code if provided
                    ...
                    visitLog = new VisitLog
                    {
                        ...
                        CreatedBy = "kiosk",
                        logStatus = true,
                        VerificationStatus = true // Auto-approve walk-ins
                    };
